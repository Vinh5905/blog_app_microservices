...
**Nghiệm thu còn thiếu:** CI PR sạch xanh, PR có CVE hoặc cấu hình HIGH đỏ, ngoại lệ hết hạn đỏ, năm image build/scan trên GitHub Actions, image lỗi chặn push, GHCR digest/SBOM/signature/release manifest đúng SHA, ruleset thật chặn merge. Chỉ cập nhật `docs/CHANGELOG.md` là hoàn tất khi có URL run và kết quả thực tế.
